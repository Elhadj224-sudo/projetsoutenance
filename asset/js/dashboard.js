...
async function chargerActualites() {
  ...
  if (newsForm) {
    newsForm.addEventListener("submit", async e => {
      ...
      try {
        const res = await fetch(url, { method, credentials: "include", headers: getAuthHeaders(), body: formData });
        if (!res.ok) throw new Error(`Erreur ${res.status}`);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if (settingsBtn) {
      settingsBtn.addEventListener("click", () => {
        window.location.href = "settings.html"; // redirection
      });
    }
  });
// pour les dons
  async function fetchDons() {
    try {
      ...
      const output = document.getElementById('dons-output');
      output.innerHTML = dons.map(don => `
        <div class="don-message">
          <strong>${don.nom} ${don.prenom}</strong> (${don.email} | ${don.telephone})<br>
          <em>Nature du don :</em> ${don.nature}<br>
          <p>${don.message || '-'}</p>
  ...
  // Onglet Messages de dons
document.getElementById('dons-tab').addEventListener('click', () => {
  // Masquer toutes les sections
  document.querySelectorAll('.tab-content').forEach(sec => sec.classList.remove('active'));
  
  // Afficher la section dons
  document.getElementById('dons-section').classList.add('active');

  // Exécuter la fonction fetchDons
  fetchDons();
});
